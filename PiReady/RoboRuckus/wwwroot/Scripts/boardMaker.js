...
// Call to generate pritnable image
function makePrintableImage() {
    // Add board date to form
    $("#boardData").val(buildBoard());
    // Add corner date to form
    $("#cornerData").val(getCorners());
    let newBoardObjRaw = $('#boardMakerForm').serializeArray();
    let newBoardObj =
    {
        selBoard: newBoardObjRaw[0].value,
        name: newBoardObjRaw[1].value,
        x_size: newBoardObjRaw[2].value,
        y_size: newBoardObjRaw[3].value,
        boardData: newBoardObjRaw[4].value.replace(/\r|\n/g, ""),
        cornerData: newBoardObjRaw[5].value
    };

    alert("Generating printable image, please be patient, this may take several minutes.");
    $.ajax({
        url: "/Setup/makePrintableBoard",
        method: "POST",
        data: JSON.stringify(newBoardObj),
        contentType: 'application/json; charset=UTF-8',
        dataType: "text"
    })
        .done(function (response) {
            if (response === "OK") {
                alert("Image sucessfully generated.");
            } else {
                alert("Problem with image generation.");
            }
