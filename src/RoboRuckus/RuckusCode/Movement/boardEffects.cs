        ...
        public static void moveConveyors(bool express)
        {
            lock(gameStatus.locker)
            {
                ...
                foreach (ConveyorModel findMove in moved)
                {
                    ...
                    moveCalculator.resolveMove(findMove.bot, findMove.space.exit, 1, ref orders, true);
                    ...
                    switch (desiredFacing - findMove.bot.currentDirection)
                    {
                        case 3:
                        case -1:
                            orders.Add(new orderModel { botNumber = findMove.bot.robotNum, move = moveCalculator.movement.Right, magnitude = 1 });
                            findMove.bot.currentDirection = desiredFacing;
                            break;
                        case -3:
                        case 1:
                            orders.Add(new orderModel { botNumber = findMove.bot.robotNum, move = moveCalculator.movement.Left, magnitude = 1});
                            findMove.bot.currentDirection = desiredFacing;
                            break;
                        case 2:
                        case -2:
                            Random rand = new Random();
                            orders.Add(new orderModel { botNumber = findMove.bot.robotNum, move = (moveCalculator.movement)rand.Next(0, 2), magnitude = 2 });
                            findMove.bot.currentDirection = desiredFacing;
                            break;
